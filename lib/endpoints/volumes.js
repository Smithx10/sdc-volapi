...
var NO_COMPUTE_RESOURCES_AVAILABLE = 'No compute resources available';
var VOLUME_TICKETS_SCOPE = 'nfs_volume';

// Type of nfs server:
//  - version 1 is a node NFS running in a zone
//  - version 2 is a native NFS running in a zone
var NFS_VERSION_1 = 1;
var NFS_VERSION_2 = 2;
...
function _buildStorageVMPayload(version, volumeParams, storageVmUuid,
    ...
    var payload = {
        uuid: storageVmUuid,
        image_uuid: CONFIG.nfsServerImageUuid,
    ...
    if (version === NFS_VERSION_2) {
        payload.brand = 'joyent';
        payload.image_uuid = CONFIG.nfsServerImageUuid2;
        payload.internal_metadata['volapi-nfs-version'] = 2;
    }
...
function createVolume(req, res, next) {
    ...
    if (req.params.tier!= undefined) {
        volumeParams['tier'] = req.params.tier;
    } 
    ...
    vasync.pipeline({funcs: [
            ...
            getAllNfsSharedVolumesPackages({
                ...
                if (requestedSize === undefined) {
                    ...
                    for (idx = 0; idx < pkgs.length; ++idx) {
                        ...
                        // Maintain backwards compatibility pkg.group 
                        // undefined = "default"
                        if (storageVmPkg.group === undefined) {
                            storageVmPkg.group = "default";
                        }
                        if (storageVmPkg.quota === requestedSize && 
                                storageVmPkg.group === volumeParams.tier) {
        ...
        function createStorageVM(ctx, done) {
            ...
            function _createStorageVmWithVersion(version, cb) {
                if (version === NFS_VERSION_2 &&
                        CONFIG.nfsServer2Enabled !== true) {
                    // Returning this error will allow fallback to version 1.
                    cb(new Error(NO_COMPUTE_RESOURCES_AVAILABLE));
                    return;
                }

                var storageVmPayload =
                    _buildStorageVMPayload(version, volumeParams,
                        ctx.storageVmUuid, ctx.storageVmPkg);
                req.log.debug({vmPayload: storageVmPayload, version: version},
                    'Creating storage VM');
...
function formatVolumeSizes(volumePkgs) {
    ...
    return volumePkgs.map(function formatPkg(pkg) {
        assert.object(pkg, 'pkg');
        assert.number(pkg.quota, 'pkg.quota');
        assert.optionalString(pkg.group, 'pkg.group');

        // To Maintain Backwards compatibility we will
        // always treat pkg.group = undefined as "default"
        // sdcadm post-setup template will be changed to set
        // a group name.
...
function checkNfsServerImgImported(req, res, next) {
    ...
    if (APPLICATION_STATE.nfsServerImageImported !== true ||
            APPLICATION_STATE.nfsServerImage2Imported !== true) {
